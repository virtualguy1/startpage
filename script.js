...
function performSearch(query) {
  ...
  categories.forEach(cat => {
    ...
    items.forEach(item => {
      ...
      const matches = !lowerQuery || slug.includes(lowerQuery) || display.includes(lowerQuery);
...
function initSearch() {
  ...
  input.addEventListener('keydown', e => {
    ...
    if (!hasVisible) {
      ...
      updateURL('');
      // Reset filter state
      document.querySelectorAll('.link-item.hidden').forEach(el => el.classList.remove('hidden'));
